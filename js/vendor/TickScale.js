...
var TickScale = (function () {
  ...
  //density = returning roughly the number of labels that were requested
    ...
    //this implementation of density is the one from the paper; problem: labelMin > dataMin & labelMax < dataMin => not useful for our implementation
/*    TickScale.prototype.density = function (k, m, dataMin, dataMax, labelMin, labelMax) {
        var roh = k / (Math.max(labelMax, dataMax) - Math.min(labelMin, dataMin));
        var roh_t = m / (Math.max(labelMax, dataMax) - Math.min(labelMin, dataMin));
        return 1 - Math.max( roh/roh_t, roh_t/roh );
    };
    TickScale.prototype.density_max = function (k, m) {
        if (k >= m)
            return 1 - (k / m);
